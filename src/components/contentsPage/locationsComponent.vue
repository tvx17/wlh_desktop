...
const m = {
  load:    async() => {
    v.value.locations = await requests.getAll(tables.Locations);
  },
  new:     () => {
    $q.dialog({
                title:      'Neuer Ort',
                message:    'Name des neuen Ortes',
                prompt:     {
                  model: '',
                  type:  'text' // optional
                },
      ...
      await requests.new(tables.Locations, {
        name:   data,
        active: true
      });
      await m.load();
  ...
  display: (id) => {
    ...
    v.value.location   = v.value.locations[id];
    v.value.dialog     = true;
  },
  save:    () => {
    v.value.locations[v.value.selectedId] = v.value.character;
    v.value.dialog                        = false;
  },
  delete:  (id) => {
    $q.dialog({
                title:      'Charakter löschen',
                message:    `Möchtest Du wirklich den Charakter mit dem Namen ${v.value.locations[id].name} löschen?`,
                cancel:     true,
                persistent: true
              }).onOk(() => {
      requests.delete(tables.Locations, v.value.locations[id].id);
      v.value.locations.splice(id, 1);
    });
  }
};

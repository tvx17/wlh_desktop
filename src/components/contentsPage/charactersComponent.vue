...
const m = {
  load:    async() => {
    v.value.characters = await requests.getAll(tables.Characters);
  },
  new:     () => {
    $q.dialog({
                title:      'Neuer Charakter',
                message:    'Name des neuen Charakters',
                prompt:     {
                  model: '',
                  type:  'text' // optional
                },
      ...
      await requests.new(tables.Characters, {
        name:   data,
        active: true
      });
      await m.load();
  ...
  display: (id) => {
    ...
    v.value.character  = v.value.characters[id];
    v.value.dialog     = true;
  },
  save:    () => {
    v.value.characters[v.value.selectedId] = v.value.character;
    v.value.dialog                         = false;
  },
  delete:  (id) => {
    $q.dialog({
                title:      'Charakter löschen',
                message:    `Möchtest Du wirklich den Charakter mit dem Namen ${v.value.characters[id].name} löschen?`,
                cancel:     true,
                persistent: true
              }).onOk(() => {
      requests.delete(tables.Characters, v.value.characters[id].id);
      v.value.characters.splice(id, 1);
    });
  }
};

<template>
  <q-dialog v-model="v_dialog_visible" persistent>
    <q-card>
      <q-card-section>Objekte: {{v_data.name}}</q-card-section>
      <q-separator/>
      <q-card-section>
...
const m = {
  load: async() => {
    // v_data.value = await requests.getBy(tables.Objects, 'id', props.id);
  },
  save: async() => {
    // await requests.update(tables.Objects, props.id, v_data.value);
    emits('visible', false);
  }
};
